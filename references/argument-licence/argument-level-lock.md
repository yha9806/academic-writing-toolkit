...
evidence. This is an optional Advisory decision aid for the existing review and audit
skills. Use it for a claim-scope review, not as a prerequisite for ordinary
copy-editing. It neither creates a new skill nor judges scientific truth.

Reuse existing project IDs, relations, evidence status, boundaries, and focus
actions when present. If the project has no ledger, use manuscript sections
and source anchors in the report; do not create standing CSV files. Record unknowns as `unknown`; never
...
For a project already using the legacy schema, preserve its
`contribution_focus.csv` `current_role` values: `primary`, `secondary`, `enabling`, `boundary`,
...
| `argument_design` | Claim hierarchy, denominator, estimand, evidence mapping, uncertainty, contribution role, or generalisation level is unclear or misaligned. | Repair the argument map in the current report before editing prose. |
...
Add this compact block to the requested review or audit report:
...
repair only adds defensive complexity. In enforced dsh mode, approvals remain harness events. A report field
or legacy CSV approval value is display only and cannot authorise an edit.
