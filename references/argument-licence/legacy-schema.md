...
> Historical schema/check interpretation, preserved from the source branch.
> Read this only when inspecting an existing packet. It does not require a
> new packet, revive a retired skill, or supply an executable validator.
> Old command and skill names below describe the historical format.
> All semantic findings are Advisory; approval fields are display only.
