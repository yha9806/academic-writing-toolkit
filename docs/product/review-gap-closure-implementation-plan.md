...
Spec: `docs/specs/2026-09-25-review-gap-closure.md` (implemented, not verified). Each step lands with a probe or test that was run on the
...
| 4 | 4.3 | `tally-readers.py`: repeat-run spread per point, `--compare` inside the noise, model-family grouping, blank reader; mechanical repetition between the abstract and the first introduction paragraph (recall rank not recorded: stated as a limit) | unit tests on synthetic outputs | dbb4c24 |
...
| 6 | 4.3 | Copyable directed questions flagged (answer keys never reach readers); a derived count coded only by the reviser reported as uncoded | unit tests | 28ff129 |
| 7 | 4.2, 4.4 | Stacked on `feat/scan-coverage` (f1abfb6): scoped `必须出现`, `承载` listing, universal-negation and "A sig, B not" flags, method sentences beside remaining cues (figure text too), required gates | tests red before | feat/ledger-questioned ec951f6 |
