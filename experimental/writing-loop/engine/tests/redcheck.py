...
MUTATIONS = [
    ...
    ("cov", 'hooks/loop_hook.py', '    if is_envelope(prompt, willow_rule()[0]):', '    if is_envelope(prompt, None):',
     'test_hooks.PromptTest.test_the_envelope_rule_is_read_from_the_willow_plugin'),
    ("cov", 'hooks/loop_hook.py', '    return t.strip() == ""', '    return True',
     'test_hooks.PromptTest.test_the_envelope_rule_is_read_from_the_willow_plugin'),
    ("cov", 'hooks/loop_hook.py', '    if willow_rule()[0] is None:\n', '    if False:\n',
     'test_hooks.PromptTest.test_without_the_willow_rule_file_the_built_in_rule_is_used_and_said'),
    ("cov", 'hooks/loop_hook.py', '        line = V.live_line(ws, cfg)', '        line = None',
    ...
    # The manuscript ring (spec W1-W4 plus the design stage): each judgement taken out must turn a test red.
    ('ring', 'ring.py', 'GATE_WORDS = [("design", ("意图卡", "设计")), ("readers", ("读者组",)), ("review", ("核对", "核完")), ("rewrite", ("改稿",))]',
     'GATE_WORDS = [("design", ("意图卡", "设计")), ("readers", ("读者组",)), ("rewrite", ("改稿",)), ("review", ("核对", "核完"))]',
     'test_ring.RingTest.test_the_gate_text_hangs_an_open_item_on_its_stage'),
    ('ring', 'ring.py', 'if stage_of(d.get("gate")) == "review" and d.get("decided_on")]', 'if d.get("decided_on")]',
    ...
    ('ring', 'ring.py', '    last = max(moves, key=lambda kt: _utc(kt[1])) if moves else None', '    last = min(moves, key=lambda kt: _utc(kt[1])) if moves else None',
    ...
    ('ring', 'lintel.py', 'if ring is not None and not ring.get("error") and not (problems or clock or flagged):',
     'if ring is not None and not ring.get("error") and not (problems or clock):',
     'test_lintel.RingExportTest.test_alerts_keep_their_own_pill'),
    ('ring', 'lintel.py', '        a["pillUntilSeen"] = False', '        a["pillUntilSeen"] = a["pillUntilSeen"]',
     'test_lintel.RingExportTest.test_the_pill_says_the_draft_and_what_waits_on_you_and_stays_after_seen'),
    ('ring', 'lintel.py', '_fit(ws, RING_PILL_MAX - width(tail)) + tail', 'ws + tail',
     'test_lintel.RingExportTest.test_the_pill_says_the_draft_and_what_waits_on_you_and_stays_after_seen'),
    ('ring', 'lintel.py', '    except Exception as e:  # noqa: BLE001 -- the card still goes up; the ring says it could not be computed',
     '    except ZeroDivisionError as e:  # mutated',
     'test_lintel.RingExportTest.test_no_coverage_no_ring_and_a_ring_that_cannot_be_computed_says_so'),
    ('ring', 'lintel.py', '        popup = [("改了", sections(lc), "primary", 1)]\n        if lc.get("basis"):',
     '        popup = [("你说", lc["verbatim"] or "", "primary", 1), ("改了", sections(lc), "primary", 1)]\n        if lc.get("basis"):',
     'test_lintel.OneActivityTest.test_a_traced_change_is_active_not_attention'),
    ('ring', 'lintel.py', '    # 「你说」「Claude 读成」不再画：对话层只有一份，由许愿柳说（#28，作者 09-24 认可）。\n',
     '    # mutated\n    pages.append({"kind": "section", "title": "你说", "items": []}) if lc["traced"] else None\n',
     'test_lintel.OneActivityTest.test_expanded_card_shows_your_words_the_reading_and_the_rows'),
    ('ring', 'ring.py', 'sorted(by_date.items(), reverse=True)', 'sorted(by_date.items())',
    ...
    ('outlet', 'hooks/loop_hook.py', '    if (said or "") == want:\n        return ""', '    if True:\n        return ""',
     'test_hooks.OutletTest.test_a_note_whose_line_is_out_of_date_is_corrected_here'),
    ('outlet', 'hooks/loop_hook.py', '    return isinstance(rule, dict) and rule.get("inbox") == 1', '    return isinstance(rule, dict)',
     'test_hooks.OutletTest.test_without_the_outlet_key_this_hook_speaks_for_itself'),
    ('outlet', 'hooks/loop_hook.py', '        spawn(ws, "git")\n        refresh_note(ws, cfg, now)', '        spawn(ws, "git")',
     'test_hooks.OutletTest.test_a_commit_refreshes_the_line_before_the_next_prompt'),
    ('outlet', 'hooks/loop_hook.py', '    want = ((head + line) if line else "") if role == "history" else (line or "")', '    want = line or ""',
     'test_hooks.OutletTest.test_a_history_session_is_noted_as_history'),
    ('outlet', 'hooks/loop_hook.py', '        HL.record_event(ws, "hook_error", f"留言没写进许愿柳（{type(e).__name__}：{e}），写作循环这一轮自己说", now=now)\n        return None',
     '        raise',
     'test_hooks.OutletTest.test_when_the_outlet_breaks_this_hook_speaks_and_says_why'),
    ('outlet', 'coverage.py', '    refresh_outlet(ws, cfg)\n    return summary', '    return summary',
     'test_hooks.OutletTest.test_compute_refreshes_a_note_and_never_starts_one'),
    ('outlet', 'coverage.py', '    ws = Path(ws).resolve()\n    cov = reminder_line', '    cov = reminder_line',
     'test_hooks.OutletTest.test_compute_refreshes_a_note_and_never_starts_one'),
    ('outlet', 'outlet.py', '    note = read(ws)\n    if note is None:', '    note = read(ws) or {"sessions": {}}\n    if note is None:',
     'test_hooks.OutletTest.test_compute_refreshes_a_note_and_never_starts_one'),
    ('outlet', 'outlet.py', '    first = not (isinstance(cur, dict) and cur.get("role") == role)', '    first = True',
     'test_hooks.OutletTest.test_a_session_already_noted_is_said_by_willow_not_here'),
    ('outlet', 'cli.py', '        V.refresh_outlet(ws, cfg)  # the note must not keep saying the removed summary\'s line\n', '',
     'test_hooks.OutletTest.test_an_update_that_removes_the_summary_also_rewrites_the_note'),
    ('envelope', 'hooks/loop_hook.py', '    if is_envelope(prompt, willow_rule()[0]):\n', '    if False:\n',
    ...
    ("turns", 'turns.py', '            ended, error = None, False   # a touch after a stop', '            pass   # a touch after a stop',
    ...
    ("turns", 'cli.py', '    TN.record(ws, f"head:{head}", now=t)', '    pass',
     'test_turns.UpdateRecordsTest.test_update_records_its_reason_and_a_new_manuscript_head'),
    # grill 09-22: a late head line, a blocked Stop, an API error, and an old index
    ("turns", 'cli.py', '    TN.record(ws, f"head:{head}", now=t)', '    TN.record(ws, f"head:{head}")',
     'test_turns.UpdateRecordsTest.test_the_head_line_carries_the_commits_time'),
    ("turns", 'turns.py', '    for r in sorted(read(ws), key=lambda r: r["t"]):', '    for r in read(ws):',
     'test_turns.TurnTest.test_a_commit_noticed_after_the_stop_does_not_reopen_the_turn'),
    ("turns", 'turns.py', 'ENDS = {"stop": False, "stop:error": True}', 'ENDS = {"stop": False, "stop:error": True, "stop:blocked": False}',
     'test_turns.TurnTest.test_a_blocked_stop_does_not_end_the_turn_and_an_api_error_does'),
    ("turns", 'lintel.py', ' and built_at >= ended and not turn.get("error")', ' and built_at >= ended',
     'test_lintel.TurnStateTest.test_a_turn_an_api_error_ended_is_over_but_does_not_land'),
    ("turns", 'cli.py', '                    before = _head_on_disk(ws) if TN else None',
     '                    before = (X.load_summary(cfg) or {}).get("head") if TN else None',
     'test_turns.UpdateRecordsTest.test_an_index_this_engine_cannot_summarize_does_not_stop_the_rebuild'),
    ("turns", 'hooks/loop_hook.py', '        spawn(ws, "stop" if out is None else "stop:blocked")', '        spawn(ws, "stop")',
     'test_hooks.RewriteGateTest.test_a_blocked_stop_is_recorded_as_blocked_and_the_override_as_a_stop'),
    ("turns", 'hooks/loop_hook.py', '        return on_stop_failure(payload, regs, now, spawn)', '        return None',
     'test_hooks.TriggerTest.test_an_api_error_ends_the_turn_as_unfinished'),
    ...
    ("cov", 'coverage.py', '            summary += _peaks_summary(data)\n', '            pass\n',
     'test_coverage.NeverGreenTest.test_a_per_section_peak_in_a_plain_section_is_named'),
    ("cov", 'catalogue.py', '"--json", "--per-file"]', '"--json"]',
     'test_coverage.NeverGreenTest.test_both_style_checks_ask_for_per_section_rates'),
    ("cov", 'catalogue.py', '"--json",\n        "--per-file"]', '"--json"]',
     'test_coverage.NeverGreenTest.test_both_style_checks_ask_for_per_section_rates'),
    ("cov", 'coverage.py', "{'反了' if p['verdict'] == 'backwards' else '值得看'}", "{'值得看'}",
     'test_coverage.NeverGreenTest.test_a_per_section_peak_in_a_plain_section_is_named'),
    ("cov", 'skill:audit/scripts/audit-prose-structure.py', '        if t is not None:\n', '        if t:\n',
     'test_structure.StructureTest.test_a_short_file_is_listed_and_the_densest_section_is_named'),
    ("cov", 'skill:audit/scripts/audit-prose-structure.py', '            per_file[name] = {"short": True,', '            pass; _ = {"short": True,',
     'test_structure.StructureTest.test_a_short_file_is_listed_and_the_densest_section_is_named'),
    ("cov", 'skill:audit/scripts/audit-prose-structure.py', '        densest = {"metric": "sub_per_comma", "file": top, "value": judged[top]["sub_per_comma"]}', '        densest = None',
     'test_structure.StructureTest.test_a_short_file_is_listed_and_the_densest_section_is_named'),
    ("cov", 'coverage.py', "        bits.append(f\"从句/逗号最高 {d['file']}（{d['value']:.2f}，只作描述）\")", '        pass',
     'test_coverage.NeverGreenTest.test_the_densest_section_of_the_structure_run_is_named_as_description'),
    ("cov", 'coverage.py', '               "readers_scope": _readers_scope(cfg, sentences),\n', '',
     'test_coverage.ShownTest.test_the_summary_records_the_reader_panel_scope'),
    ("cov", 'coverage.py', "        bits.append(f\"读者组只读 {'、'.join(rs['sections'])}{share}\")", '        pass',
     'test_coverage.ShownTest.test_the_line_says_which_sections_the_reader_panel_reads'),
    ("cov", 'coverage.py', '    per = _per_section_bits(summary)\n', '    per = []\n',
    ...
    ("state", 'state.py', '    if labels:\n        blockers.append', '    if False:\n        blockers.append',
     'test_state.StateTest.test_an_overclaim_anywhere_in_the_draft_is_found_though_nothing_changed'),
    ("state", 'state.py', '            if not any(rx.search(s.get("text") or "") for s in sentences):', '            if False:',
     'test_state.StateTest.test_a_required_wording_that_is_gone_is_said'),
    ("state", 'state.py', 'next=[t["id"] for t in open_[:3]]', 'next=sorted(t["id"] for t in open_)[:3]',
     'test_state.StateTest.test_open_work_blocks_and_the_next_items_follow_the_ledger'),
    ("state", 'state.py', 'CLOSED = re.compile(r"^(已做|不做)\\s+(\\d{4}-\\d{2}-\\d{2})\\s+(\\S.*)$")',
     'CLOSED = re.compile(r"^(已做|不做)()()")',
     'test_state.StateTest.test_a_closed_item_needs_a_date_and_evidence'),
    ("state", 'state.py', '        st["problems"].append(f"主张清单读不到：{p}")', '        pass',
     'test_state.StateTest.test_an_unreadable_ledger_is_not_read_as_fine'),
    ("state", 'state.py', '        st["problems"].append("句子索引没建', '        None and st["problems"].append("句子索引没建',
     'test_state.StateTest.test_an_index_that_was_never_built_is_said'),
    ("state", 'coverage.py', '    return paper + ("。" + cov if cov else "")', '    return cov',
     'test_state.StateTest.test_without_a_ledger_the_line_says_the_loop_does_not_know'),
    ("state", 'overview.py', '                Path(self.cfg["_ws"]) / "cache" / "coverage" / "summary.json"] + claims',
