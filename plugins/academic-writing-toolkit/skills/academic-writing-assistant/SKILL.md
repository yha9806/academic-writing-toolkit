...
description: Orchestrate project-centred academic writing by locating the canonical project state, mapping the research spine, selecting the next specialist workflow, and preserving author control across gaps, data, results, claims, contributions, innovation evidence, integration, reframing, and revision. Use when the user asks to 梳理研究项目、项目主线、目前有什么、下一步做什么、内容或证据是否过多或过少、是否需要调整贡献侧重, or asks for an academic writing assistant to coordinate several toolkit skills. Do not use as a rebuttal workflow or for one mechanical style fix.
---
...
3. Select exactly one primary specialist route for the current turn.
4. Read and follow that specialist skill, including its approval and stop gates.
5. Preserve its object IDs, rule IDs, evidence status, and uncertainty.
6. Never turn a candidate contribution focus into a manuscript change without
   explicit author approval.
7. Record inspected, changed, deliberately unchanged, decided, and deferred
   items. Do not record hidden reasoning, credentials, or sensitive text.
8. Match the report language to the user's language. Keep canonical object IDs,
