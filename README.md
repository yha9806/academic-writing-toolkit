...
| **Long-document Workbench** | optional Python component for document import, bounded reviews, resumable jobs, revision reuse, layout comparison and submission checks | Advisory review with local validation; independent of dsh guards |

The two modes remain Enforced or Advisory. The optional Workbench is an
explicit [2026-09-05 integration](docs/specs/2026-09-05-long-document-integration.md)
of the long-document work; its review outputs are advisory. The packaged
Codex plugin and ChatGPT App remain retired, with their historical releases
listed under [Release and distribution](#release-and-distribution).
...
## Review a long thesis in the Workbench

The optional [Workbench component](workbench/README.md) imports PDF/DOCX,
plans small-context review batches, saves progress, reuses unchanged revision
batches, compares pages locally and checks manuscript/outline/material rules.
Install it in a dedicated Python 3.9+ environment from this repository root:
...
Open `http://127.0.0.1:8784/project`. See the
[超长文档 guide](workbench/docs/thesis-scale-review.md) for budgets and recovery.
The configured input ceiling is 1,000 PDF pages; the scale experiment uses
200 synthetic pages, mock model reviews and real local rendering. These
engineering checks do not establish review quality or replace the E1/E2
evidence. The Workbench does not mount the dsh guard/approval system.
...
├── workbench/               optional long-document Python package, UI, tests and scale evidence
...
- [v0.5.0 stable release](https://github.com/yha9806/academic-writing-toolkit/releases/tag/v0.5.0)
  ...
  the Cloud Run/Render deployments, and the original local workbench wheel.
  The App and deployments remain decommissioned. The optional `workbench/`
  source component is now version `0.5.1.dev0`, with no release/publication claim.
