...
### 4.5 Audit Experiment And Analysis Admission

If the project uses the lightweight `/thesis-control` profile, reuse the
`Experiment and analysis admission` table in
`01_EVIDENCE_AND_CLAIMS.md`; do not create a competing register. Otherwise,
cross-check analysis objects and relations in the strict argument packet and
report the same fields in the governance output:

- directly answers the core question: `yes`, `no`, or `partial`
- main conclusion survives removal: `yes`, `no`, or `unknown`
- role: `primary`, `explanatory`, `robustness`, `development_record`, or `out_of_scope`
- destination: `main_text`, `supplementary`, `internal_record`, or `exclude`
- one-sentence argumentative function
- evidence anchor and author decision

Use role-to-destination defaults from `/thesis-control`. Require an explicit
author decision before promoting or demoting an analysis. A complete analysis
with no distinct argumentative function is an excess candidate, not an
...
| Analysis | Direct answer | Main conclusion survives removal | Role | Destination | Function | Evidence anchor | Author decision |
...
- intended use and current validation cannot be separated from the available project artifacts
- an analysis is promoted to primary without a distinct core-question function and author decision
