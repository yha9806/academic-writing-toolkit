// .claude/skills/audit/scripts/audit-citation-fidelity.mjs: the audit's four
// findings pinned on temp workspaces, plus the honesty test — a citing
// sentence that INVERTS its source in the source's own words, which the tool
// is REQUIRED to pass without a finding, while its output states that limit.
// The limit is a tested claim, not a README sentence. Moved here from the
// retired dsh guards package on 2026-09-20 with the audit's dependencies.
...
function workspace(chapter, notes = notesFixture()) {
...
const kinds = (out) => out.findings.map((f) => f.kind).sort()
...
test('a LaTeX manuscript is reported as not covered, not as clean', () => {
  ...
  assert.equal(out.not_covered?.latex_files_with_cites, 1)
  assert.equal(status, 2)
})
