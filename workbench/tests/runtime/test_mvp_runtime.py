...
""",
            ...
            if os.name == "nt":
                fake_codex = Path(temporary) / "codex.cmd"
                fake_codex.write_text(
                    '@echo off\n'
                    'if "%*"=="--version" (echo codex 0.test & exit /b 0)\n'
                    'if "%*"=="login status" (echo Logged in using test fixture & exit /b 0)\n'
                    'if "%*"=="exec --help" (echo --ephemeral --ignore-rules --ignore-user-config --output-last-message --output-schema --sandbox --skip-git-repo-check & exit /b 0)\n'
                    'exit /b 2\n',
                    encoding="ascii",
                )
            environment = {
                **{key: value for key, value in os.environ.items() if not key.startswith("AWT_")},
                "AWT_PROVIDER": "codex",
            ...
            self.assertEqual(version.stdout.strip(), "mvp.py 0.5.1.dev0")
    ...
    def test_codex_runner_must_be_executable(self):
        if os.name == "nt":
            self.skipTest("Windows does not implement POSIX executable permission bits")
        with tempfile.TemporaryDirectory() as temporary:
