...
class ThesisJobsTests(unittest.TestCase):
    ...
    def create(self, input_tokens=4000):
        return self.manager.create({"files":self.files(), "goal":"Check consistency",
            "budget":{"input_tokens":input_tokens, "max_calls":500, "total_tokens":3000000}})
    ...
    def test_hierarchy_has_grounded_chapter_reviews_and_complete_navigation(self):
        # Exercise chapter aggregation with enough text batches under either
        # the optional tokenizer or the conservative dependency-free estimator.
        result = self.run_job(self.create(input_tokens=2400)["id"])
        ...
        for node in nodes:
            self.assertEqual(node["checked_blocks"], node["total_blocks"], node["title"])
            self.assertTrue(node["summaries"], node["title"])
    ...
    def test_one_paragraph_revision_reuses_most_text_and_invalidates_dependents(self):
        self._check_single_paragraph_revision()

    def test_revision_reuse_with_denser_token_estimates(self):
        # A dependency or tokenizer change can fit more source blocks in one
        # request. That must not change which complete batches remain reusable.
        with patch("awt.cross_review.estimate_tokens", side_effect=lambda text:
                   {"tokens": math.ceil(len(text.encode("utf-8")) / 5)}):
            self._check_single_paragraph_revision()

    def test_complete_coverage_when_chapter_aggregation_is_unnecessary(self):
        result = self.run_job(self.create(input_tokens=16000)["id"])
        nodes = self.manager.page({"job_id":result["id"], "kind":"chapters"})["items"]
        self.assertEqual({step["phase"] for step in result["steps"]}, {"text", "cross"})
        for node in nodes:
            self.assertEqual(node["checked_blocks"], node["total_blocks"], node["title"])
            self.assertEqual(node["summaries"], [], node["title"])

    def _check_single_paragraph_revision(self):
        job = self.create()
        first = self.run_job(job["id"])
        old_count = len(self.calls)
        original_steps = copy.deepcopy(self.manager._load(job["id"])["steps"])
        source = next(d for d in first["documents"] if d["filename"] == "Results.md")
        changed_id = next(b["id"] for b in source["blocks"] if b["text"].startswith("Paragraph 40:"))
        affected = next(s for s in original_steps if s["phase"] == "text" and changed_id in s["block_ids"])
        total = sum(len(d["blocks"]) for d in first["documents"])
        ...
        # The whole changed batch is invalidated; every other block's batch
        # must be reused. An absolute reuse threshold depends on tokenization.
        self.assertEqual(second["revision_change"]["unchanged_blocks"], total - 1)
        self.assertEqual(second["revision_change"]["reused_blocks"], total - len(affected["block_ids"]))
        self.assertEqual(second["revision_change"]["pending_blocks"], len(affected["block_ids"]))
